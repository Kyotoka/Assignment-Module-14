...
const Card = () => {
    const companyLogos = [
      ...
      return (
        <div className='bg-blue-200 pt-5 pb-5 mt-5'>
            <div className="flex flex-col lg:flex-row justify-around rounded-md overflow-hidden mx-4 my-8 lg:mx-auto lg:w-3/4 xl:w-full p-15 ">
                ...
                <div className="w-96 mx-10 flex flex-col justify-center">
                    <h2 className="text-3xl lg:text-4xl font-bold mb-4 text-white">
                        Earn Customers Loyalty and Satisfaction
                    </h2>
                    <p className="text-base lg:text-lg text-gray-600">
                        We help businesses like yours to gather more customers, outstand other competitors, make more profit.
                    </p>
                ...
                <div className="flex justify-evenly p-10">
                    {
                        companyLogos.map((item) => {
                            return (<Link to={item.url} className='font-bold text-lg text-yellow-500' key={item.name} target="_blank">{item.name}</Link>)
                        })
                    }
                </div>
            </div>
